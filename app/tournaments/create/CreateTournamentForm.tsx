...
function defaultTargets(name: string) {
  if (name === 'five_stand') return 25      // 5 stations × 5 birds
  if (name === 'super_sport') return 50     // 5 stations × 10 birds (common)
  if (name === 'sporting_clays') return 100 // 10 stations × 10 birds (common)
  return 0
}
...
export default function CreateTournamentForm({ disciplines }: CreateTournamentFormProps) {
  ...
  // Per-discipline round/station/targets config
  const [disciplineConfigs, setDisciplineConfigs] = useState<Record<string, { rounds: number; stations: number; targets: number }>>(() => {
    ...
    for (const d of disciplines) {
      map[d.id] = { rounds: defaultRounds(d.name), stations: defaultStations(d.name), targets: defaultTargets(d.name) }
    }
    return map
  })
